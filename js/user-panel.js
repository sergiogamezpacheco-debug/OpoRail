...
function saveUserLocal(user){ localStorage.setItem(STORAGE_KEY, JSON.stringify(user)); }
function loadUserLocal(){ try { return JSON.parse(localStorage.getItem(STORAGE_KEY)) || null; } catch(e){ return null; } }
function clearUserLocal(){ localStorage.removeItem(STORAGE_KEY); }
...
function renderProfileCard(){
  ...
  if(avatarEl){ avatarEl.style.backgroundImage = user?.avatar ? `url(${user.avatar})` : ''; }
}
...
function loadView(view){
  ...
  if(view==='dashboard'){
    content.innerHTML = `<h2>Bienvenido, ${state.user.name}</h2>
      ...
      <div style="margin-top:16px"><a class="btn" href="profile.html">Ir a Perfil</a></div>`;
...
function initSettings(){
  ...
  dm.addEventListener('change', ()=>{
    if(dm.checked){ localStorage.setItem('oporail_darkmode','1'); document.documentElement.classList.add('dark'); }
    else { localStorage.removeItem('oporail_darkmode'); document.documentElement.classList.remove('dark'); }
  });
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(document.getElementById('content')){
    if(state.user){ loadView('dashboard'); }
    else { loadView('auth'); }
    attachAuthHandlers();
  }
});
